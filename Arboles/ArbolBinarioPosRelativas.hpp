...
public:
	...
	explicit Abin(int maxNodos, const T& e_nulo = T()); //ctor
...
template <typename T>
Abin<T>::Abin(int maxNodos, const T& e_nulo) :
...
void Abin<T>::eliminarHijoIzqdoB(Abin<T>::nodo n)
{
	...
	nodos[4*n+4] == ELTO_NULO); // n es hoja
		...
		if(4*n+3 < maxNodos) //sólo cabe h. izq. de h. izq. de n
...
void Abin<T>::eliminarHijoDrchoB(Abin<T>::nodo n)
{
	...
	nodos[4*n+6] == ELTO_NULO); // n es hoja
		...
		if(4*n+5 < maxNodos) //sólo cabe h. izq. de h. drch de n
